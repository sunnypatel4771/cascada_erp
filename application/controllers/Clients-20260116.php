...
class Clients extends ClientsController
{
    ...
    public function index()
    {
        ...
        // Fetch invoice items if order exists
        $latest_order_items = [];
        if ($latest_order) {
            $latest_order_items = $this->db->where('rel_id', $latest_order->id)
        ...
        // Fetch all available products/items for the catalog
    ...
    public function save_new_order()
    {
        ...
        foreach ($items as $index => $item) {
            ...
            $newitems[] = [
                ...
                'order' => isset($item['order']) ? $item['order'] : ($index + 1), // Add order field
            ];
        }
