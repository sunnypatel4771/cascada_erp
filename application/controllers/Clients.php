...
class Clients extends ClientsController
{
    ...
    public function index()
    {
        ...
        // Ensure ripeness column exists
        $this->_ensure_ripeness_column();

        $client_id = get_client_user_id();

        // Fetch latest invoice/order for this customer (order by id DESC for reliable latest)
        $latest_order = $this->db->where('clientid', $client_id)
                                ->order_by('id', 'DESC')
                                ->limit(1)
                                ->get(db_prefix() . 'invoices')
                                ->row();

        // Fetch invoice items if order exists (with product images)
        $latest_order_items = [];
        if ($latest_order) {
            // Get items with product images via subquery
            $files_table = db_prefix() . 'files';
            $itemable_table = db_prefix() . 'itemable';
            $items_table = db_prefix() . 'items';

            // Subquery to get only the latest image per item (highest id)
            $this->db->select("{$itemable_table}.*, img.file_name as image_file, {$items_table}.id as item_id");
            $this->db->from($itemable_table);
            $this->db->join($items_table, "{$items_table}.description = {$itemable_table}.description", 'left');
            $this->db->join(
                "(SELECT f1.rel_id, f1.file_name
                  FROM {$files_table} f1
                ...
                "img.rel_id = {$items_table}.id",
                'left'
            );
            $this->db->where("{$itemable_table}.rel_id", $latest_order->id);
            $this->db->where("{$itemable_table}.rel_type", 'invoice');
            $latest_order_items = $this->db->get()->result();

            // Build image URLs for each item
            $default_image = base_url('modules/warehouse/uploads/nul_image.jpg');
            foreach ($latest_order_items as &$item) {
                if (!empty($item->image_file) && !empty($item->item_id)) {
                    $image_path = 'modules/warehouse/uploads/item_img/' . $item->item_id . '/' . $item->image_file;
                    if (file_exists(FCPATH . $image_path)) {
                        $item->image_url = base_url($image_path);
                    } else {
            ...
            $currency = $this->db->where('id', $latest_order->currency)
                                ->get(db_prefix() . 'currencies')
                                ->row();
            
            if ($currency) {
                $latest_order->currency_symbol = $currency->symbol;
                $latest_order->currency_name = $currency->name;
            } else {
                $latest_order->currency_symbol = '';
                $latest_order->currency_name = '';
            }
        }

        // Fetch all available products/items for the catalog (now includes image_url from get_grouped)
        ...
        $data['available_products'] = $grouped_products;
        $data['all_products'] = $all_products;

        // Get default currency for display
        $default_currency = $this->db->where('isdefault', 1)
                                    ->get(db_prefix() . 'currencies')
                                    ->row();
        $data['currency_symbol'] = $default_currency ? $default_currency->symbol : '$';

        // Calculate previous order total
        $previous_order_total = 0;
        if (!empty($latest_order_items)) {
            foreach ($latest_order_items as $item) {
                $previous_order_total += (float)$item->qty * (float)$item->rate;
            }
        }
        ...
        $data['latest_order_items'] = $latest_order_items;
        $data['title'] = _l('clients_portal_dashboard');
    ...
    // Add new method for saving orders
    public function save_new_order()
    {
        ...
        // Ensure ripeness column exists in tblitemable
        $this->_ensure_ripeness_column();
        ...
        // Get order items from POST
        $items = $this->input->post('items');

        if (empty($items)) {
            echo json_encode(['success' => false, 'message' => _l('no_items_in_order')]);
            return;
        }
        ...
        $invoice_data = [
            'clientid' => $client_id,
            'number' => get_option('next_invoice_number'), // Set invoice number from settings
            'date' => date('Y-m-d'),
        ...
        if ($invoice_id) {
            ...
            echo json_encode([
                'success' => false,
                'message' => _l('order_save_failed')
            ]);
        }
    }

    /**
     * Ensure ripeness column exists in tblitemable table
     */
    private function _ensure_ripeness_column()
    {
    ...
    /**
     * Update ripeness for items after invoice creation
     */
    private function _update_items_ripeness($invoice_id, $items)
    {
        // Get the items that were just created for this invoice
        $invoice_items = $this->db->where('rel_id', $invoice_id)
                                  ->where('rel_type', 'invoice')
                                  ->order_by('item_order', 'ASC')
                                  ->get(db_prefix() . 'itemable')
                                  ->result();

        // Match items by description and order to update ripeness
        foreach ($invoice_items as $index => $db_item) {
            foreach ($items as $post_item) {
                if ($db_item->description === $post_item['description'] && !empty($post_item['ripeness'])) {
                    $this->db->where('id', $db_item->id)
                             ->update(db_prefix() . 'itemable', ['ripeness' => $post_item['ripeness']]);
                    break;
                }
            }
        }
