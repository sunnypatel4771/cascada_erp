...
class Picking extends AdminController
{
    ...
    public function index(): void
    {
        ...
        foreach ($inventory as $item) {
            $inventoryOptions[] = [
                'id'   => $item['id'],
                'name' => trim($item['item_name'] . ' (' . $item['unit'] . ')'),
            ];
        }
    ...
    public function start_shift($moduleId): void
    {
        ...
        $success = $this->modules_model->start_shift($moduleId, $staffId);
    ...
    public function update_item($pickId): void
    {
        if (!staff_can('edit', RAMOS_MODULE_NAME) && !is_admin()) {
            // non-managers must have active shift, check later after fetching pick
        }
        ...
        if (!is_admin() && !staff_can('edit', RAMOS_MODULE_NAME)) {
