...
hooks()->add_action('admin_init', 'omni_sales_init_customer_profile_tab');
...
/**
 * Add Orders tab to customer profile sidebar
 */
function omni_sales_init_customer_profile_tab()
{
    $CI = &get_instance();

    // Add Orders tab before Invoices (position 23, invoices is at 25)
    $CI->app_tabs->add_customer_profile_tab('omni_orders', [
        'name'     => _l('orders'),
        'icon'     => 'fa fa-shopping-cart',
        'view'     => 'omni_sales/clients/orders',
        'visible'  => (has_permission('omni_order_list', '', 'view') || has_permission('omni_order_list', '', 'view_own') || is_admin()),
        'position' => 23,
        'badge'    => [],
    ]);
}
...
function omni_sales_load_js(){
    ...
    // Initialize customer orders DataTable on client profile page
    if (!(strpos($viewuri,'/admin/clients/client/') === false)) {
        echo '<script>
        $(function() {
            if ($(".table-customer-orders").length > 0) {
                var customer_id = $("input[name=\'userid\']").val();
                if (customer_id) {
                    initDataTable(".table-customer-orders", admin_url + "omni_sales/customer_orders_table/" + customer_id, undefined, undefined, undefined, [0, "desc"]);
                }
            }
        });
        </script>';
...
function omni_sales_module_init_client_menu_items()
{
    if(get_option('omni_show_public_page') == 1){
        ...
        if($status_channel == 'active' && get_option('omni_display_shopping_cart') == 1){
            ...
            // HIDDEN: "Lista de pedidos" order list nav item removed per client requirement
        }
    }
}
/**
