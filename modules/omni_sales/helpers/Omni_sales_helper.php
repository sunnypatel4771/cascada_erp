...
function cron_job_sync_woo($store = ''){
	...
	if($store != ''){
		if($sync_omni_sales_orders == "1"){
			if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_orders_time6.' minutes', strtotime($records_time6))))){
				try {
					$result = $CI->omni_sales_model->process_orders_woo($store);
				} catch (Exception $e) {
				} finally {
				}
				update_option('records_time6', date("Y-m-d H:i:s"));
			}
		}
		if($sync_omni_sales_inventorys == "1"){
			if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_inventory_info_time2.' minutes', strtotime($records_time2))))){
				try {
					$result = $CI->omni_sales_model->process_inventory_synchronization_detail($store);
				} catch (Exception $e) {
				} finally {
				}
				update_option('records_time2', date("Y-m-d H:i:s"));
			}
		}
		if($sync_omni_sales_description == "1"){
			if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_decriptions_time4.' minutes', strtotime($records_time4))))){
				if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_orders_time6.' minutes', strtotime($records_time6))))){
					try {
					...
					update_option('records_time4', date("Y-m-d H:i:s"));
				}

			}
		}
		if($sync_omni_sales_images == "1"){
			if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_images_time5.' minutes', strtotime($records_time5))))){
				try {
					$result = $CI->omni_sales_model->process_images_synchronization_detail($store);
				} catch (Exception $e) {
				} finally {
				}
				update_option('records_time5', date("Y-m-d H:i:s"));
			}
		}
		if($price_crm_woo == "1"){
			if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_price_time3.' minutes', strtotime($records_time3))))){
				try {
					$result = $CI->omni_sales_model->process_price_synchronization($store);
				} catch (Exception $e) {
				} finally {
				}
				update_option('records_time3', date("Y-m-d H:i:s"));
			}
		}
		if($product_info_image_enable_disable == "1"){
			if(strtotime($hour) >= strtotime(date('Y-m-d H:i:s', strtotime('+'.$minute_sync_product_info_images_time8.' minutes', strtotime($records_time8))))){
				$success = $CI->omni_sales_model->sync_from_the_store_to_the_system($store, '');
				update_option('records_time8', date("Y-m-d H:i:s"));
			}
		}
	}
	return true;
...
function get_name_store($id){
	$CI = & get_instance();      
	$CI->db->where('id', $id);
	$CI->db->select('name_channel');
	$data = $CI->db->get(db_prefix().'omni_master_channel_woocommere')->row();
	if($data){
		return $data->name_channel;
	}
	return '';
}
...
/**
 * om get ordernumber by name
 * @param  [type] $order_number 
 * @return [type]               
 */
function om_get_ordernumber_by_name($order_number)
{
    $order_number_id = '';
    $CI           = & get_instance();
 ...
 * htmldecode
 */
function omni_htmldecode($str){
	return html_entity_decode($str ?? '');
}

function omni_json_decode($str){
