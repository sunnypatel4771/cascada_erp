...
class Omni_sales_client extends ClientsController
{
	...
	public function __construct()
	{
		...
		$status_channel = '';
		$data_channel = $this->db->query('select status from '.db_prefix().'sales_channel where channel="portal"')->row();
		if($data_channel){
	...
	public function index($page='',$id = '', $warehouse = '',$key = ''){  
		 ...
		 */
		 public function get_trade_discount(){
		 	...
		 	$list_discount = $this->omni_sales_model->get_discount_list($channel, $data_id);
		 	$result = [];        
