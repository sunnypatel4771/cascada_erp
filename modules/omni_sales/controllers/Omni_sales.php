...
class Omni_sales extends AdminController
{
		...
		public function pos(){
			...
			if(get_option('omni_sell_the_warehouse_assigned') == 1){
				if(omni_get_status_modules('warehouse')){
					$this->load->model('warehouse/warehouse_model');
					$data['getWarehouseByStaff'] = $this->warehouse_model->getWarehouseByStaff();
				}
			}
	...
	/**
	 * Customer orders table for customer profile tab
	 * @param int $customer_id
	 * @return json
	 */
	public function customer_orders_table($customer_id = 0){
		if (!$this->input->is_ajax_request()) {
			ajax_access_denied();
		}
		...
		$where = [' where userid = ' . $customer_id];

		// Check permissions for view_own
		if (!is_admin() && !has_permission('omni_order_list', '', 'view')) {
			array_push($where, ' and seller = ' . get_staff_user_id());
		}
		...
		foreach ($rResult as $aRow) {
			...
			$option = '';
			if (has_permission('omni_order_list', '', 'view') || has_permission('omni_order_list', '', 'view_own') || is_admin()) {
				$option .= '<a href="' . admin_url('omni_sales/view_order_detailt/' . $aRow['id']) . '" data-toggle="tooltip" data-placement="top" data-title="' . _l('view') . '" class="btn btn-default btn-icon">';
				$option .= '<i class="fa fa-eye"></i>';
				$option .= '</a>';
			}

			if (is_admin() || has_permission('omni_order_list', '', 'edit')) {
				if ($aRow['channel_id'] == 4 || $aRow['channel_id'] == 6) {
					$option .= '<a href="' . admin_url('omni_sales/order_manual/' . $aRow['id']) . '" data-toggle="tooltip" data-placement="top" data-title="' . _l('edit') . '" class="btn btn-default btn-icon">';
					$option .= '<i class="fa fa-pencil"></i>';
					$option .= '</a>';
				}
			}
	 ...
	 * get cart data 
		...
		public function get_product_by_group_pos_channel($page = '', $id = '',$warehouse = '',$key=''){ 
			...
			foreach ($arr_warehouse as $warehouse) {
				...
				$data_product_count += $data_product['count'];

			}

			$data['group_id'] = $id;
			$data['page'] = $page;
			$data['ofset_count'] = $data_product_count;
		...
		public function save_setting($type){
			...
			switch ($type) {
				...
				if (isset($data['omni_pos_shipping_fee'])) {
					update_option('omni_pos_shipping_fee', (float)$data['omni_pos_shipping_fee']);
				}else{
					update_option('omni_pos_shipping_fee', 0);
				}

				if ($data['omni_portal_shipping_fee']) {
					update_option('omni_portal_shipping_fee', (float)$data['omni_portal_shipping_fee']);
				}else{
					update_option('omni_portal_shipping_fee', 0);
				}

				if ($data['omni_manual_shipping_fee']) {
					update_option('omni_manual_shipping_fee', (float)$data['omni_manual_shipping_fee']);
				}else{
				...
				if ($data['number_of_days_to_save_diary_sync'] != '') {
					update_option('number_of_days_to_save_diary_sync', $data['number_of_days_to_save_diary_sync']);
				}else{
					update_option('number_of_days_to_save_diary_sync', 0);
				}

				if ($data['omni_sale_hide_shipping_fee']) {
					update_option('omni_sale_hide_shipping_fee', 1);
				}else{
					update_option('omni_sale_hide_shipping_fee', 0);
				}
				...
				if($data['omni_show_public_page']){
					update_option('omni_show_public_page', 1);
				}else{
					update_option('omni_show_public_page', 0);
				} 
				if($data['omni_sell_the_warehouse_assigned']){
					update_option('omni_sell_the_warehouse_assigned', 1);
				}else{
					update_option('omni_sell_the_warehouse_assigned', 0);
				} 
	...
	public function get_store_woo(){
		...
		public function create_pos_customer(){ 
			...
			if($id > 0){
				...
				foreach ($data_client as $key => $value) {
					$html .= '<option value="'.$value['userid'].'" data-subtext="'.$value['phonenumber'].'">'.$value['company'].'</option>';
				}
			}
			echo json_encode([
				'success' => $success,
		...
		public function process_asynclibrary_inventory($store_id){
			update_option('status_sync', 1);
			$success = $this->omni_sales_model->process_inventory_synchronization_detail($store_id);
			update_option('status_sync', 2);
			echo json_encode($success);
		}
    ...
    public function pdf($id)
    {
        ...
        if($data_cart){
        	if(is_admin() || $data_cart->seller == get_staff_user_id() || has_permission('omni_order_list', '', 'view')|| has_permission('omni_order_list', '', 'view_own')){
        		...
        		$this->load->model('staff_model');
        		$data['staffs'] = $this->staff_model->get('staff');   
        		$data['title'] = '#'.$data_cart->order_number;   
        		$data['activity_log'] = [];
        		if($this->db->table_exists(db_prefix() . 'wh_goods_delivery_activity_log')){
        			$data['activity_log'] = $this->warehouse_model->wh_get_activity_log($data['order']->stock_export_number,'omni_order');						
        		}
        		...
        		$data['type'] = $type;
        		$html = $this->load->view('order_list/order_detail_pdf.php', $data, true);
        		$html .= '<link href="' . module_dir_url(OMNI_SALES_MODULE_NAME, 'assets/css/cart/order_detail_pdf.css') . '"  rel="stylesheet" type="text/css" />';
        		$data['html'] = $html;
        		$this->load->view('preview_pdf', $data);
        	} 
        	else{
        		access_denied('order');
        	}
