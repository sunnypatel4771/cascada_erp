...
class Picking_model extends App_Model
{
    ...
    public function ensure_pick_records_for_module($moduleId): void
    {
        ...
        $rows = $this->db
            ...
            ->where('c.original_order_id IS NULL', null, false) // Exclude return orders
            ->get()
            ->result_array();
    ...
    public function get_orders_for_module($moduleId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!isset($orders[$orderId])) {
                $orders[$orderId] = [
                    ...
                    'route_priority'  => $row['route_priority'] ?? null,  // Route stop number for blue badge
                    'route_id'        => $row['route_id'] ?? null,        // Route ID reference
                    'items'           => [],
                ];
            }
        ...
        foreach ($orders as $order) {
            ...
            // This gives a more accurate progress indicator than just counting completed items
            $totalProgress = 0;
            foreach ($order['items'] as $item) {
                $requiredQty = (float) $item['required_qty'];
                $pickedQty = (float) $item['picked_qty'];

                if ($requiredQty > 0) {
                    $itemProgress = min(100, ($pickedQty / $requiredQty) * 100);
                    $totalProgress += $itemProgress;
                }
            }

            $order['status'] = $displayStatus;
            $order['progress'] = $totalItems > 0 ? round($totalProgress / $totalItems) : 0;
