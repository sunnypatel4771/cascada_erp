...
if (!$CI->db->table_exists(db_prefix() . 'ramos_module_staff')) {
    ...
    $CI->db->query(
        'CREATE TABLE `' . db_prefix() . "ramos_module_staff` (
            ...
            `role` ENUM('operator', 'supervisor') NOT NULL DEFAULT 'operator',
...
if ($CI->db->table_exists(db_prefix() . 'ramos_module_staff')) {
    if (!$CI->db->field_exists('role', db_prefix() . 'ramos_module_staff')) {
        $CI->db->query(
            'ALTER TABLE `' . db_prefix() . "ramos_module_staff`
             ADD `role` ENUM('operator', 'supervisor') NOT NULL DEFAULT 'operator' AFTER `staff_id`"
        );
    }
}
...
// Add route_stop_id to link pick items to delivery route stops
// This enables route-priority picking where items are picked in delivery order
if (!$CI->db->field_exists('route_stop_id', db_prefix() . 'ramos_pick_items')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . "ramos_pick_items` ADD `route_stop_id` INT UNSIGNED NULL AFTER `order_id`;");
    $CI->db->query('CREATE INDEX `idx_ramos_pick_route_stop` ON `' . db_prefix() . 'ramos_pick_items` (`route_stop_id`);');
}
...
// ===================================================================
// AUTOMATION TRACKING FEATURE - Database Updates
// ===================================================================

// Add automation tracking columns to orders table
if ($CI->db->table_exists(db_prefix() . 'ramos_orders')) {
    if (!$CI->db->field_exists('processed_for_purchase', db_prefix() . 'ramos_orders')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "ramos_orders` ADD `processed_for_purchase` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Whether order was included in purchase automation' AFTER `status`;");
    }

    if (!$CI->db->field_exists('processed_at', db_prefix() . 'ramos_orders')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "ramos_orders` ADD `processed_at` DATETIME NULL COMMENT 'When order was processed by automation' AFTER `processed_for_purchase`;");
    }

    if (!$CI->db->field_exists('automation_run_id', db_prefix() . 'ramos_orders')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "ramos_orders` ADD `automation_run_id` INT UNSIGNED NULL COMMENT 'Link to automation run' AFTER `processed_at`;");
    }

    // Create index for processed_for_purchase if it doesn't exist
    $indexes = $CI->db->query('SHOW INDEX FROM `' . db_prefix() . "ramos_orders` WHERE Key_name = 'idx_ramos_orders_processed'")->result_array();
    if (empty($indexes)) {
        $CI->db->query('CREATE INDEX `idx_ramos_orders_processed` ON `' . db_prefix() . 'ramos_orders` (`processed_for_purchase`);');
    }
}

// Add priority field to suppliers table
if ($CI->db->table_exists(db_prefix() . 'ramos_suppliers')) {
    if (!$CI->db->field_exists('priority', db_prefix() . 'ramos_suppliers')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "ramos_suppliers` ADD `priority` INT UNSIGNED NOT NULL DEFAULT 999 COMMENT 'Supplier priority (lower = higher priority, 1 is best)' AFTER `active`;");
    }

    // Create index for priority if it doesn't exist
    $indexes = $CI->db->query('SHOW INDEX FROM `' . db_prefix() . "ramos_suppliers` WHERE Key_name = 'idx_ramos_suppliers_priority'")->result_array();
    if (empty($indexes)) {
        $CI->db->query('CREATE INDEX `idx_ramos_suppliers_priority` ON `' . db_prefix() . 'ramos_suppliers` (`priority`);');
    }
}
...
if (!$CI->db->table_exists(db_prefix() . 'ramos_automation_runs')) {
    ...
    $CI->db->query(
        'CREATE TABLE `' . db_prefix() . "ramos_automation_runs` (
            `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
            `run_type` VARCHAR(50) NOT NULL DEFAULT 'purchase_generation' COMMENT 'Type of automation',
            `total_orders_processed` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Number of orders included',
            `total_purchase_orders_created` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Number of POs generated',
            `status` VARCHAR(20) NOT NULL DEFAULT 'running' COMMENT 'Status: running, completed, failed',
            `run_by` INT UNSIGNED NULL COMMENT 'Staff user ID who triggered',
            `run_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'When automation started',
            `completed_at` DATETIME NULL COMMENT 'When automation completed',
            `notes` TEXT NULL COMMENT 'Error messages or notes',
            `summary` TEXT NULL COMMENT 'JSON summary of results',
            PRIMARY KEY (`id`),
            KEY `idx_ramos_automation_runs_type` (`run_type`),
            KEY `idx_ramos_automation_runs_status` (`status`),
            KEY `idx_ramos_automation_runs_date` (`run_at`)
        ) ENGINE=InnoDB DEFAULT CHARSET={$charset} COLLATE={$collation}
...
// This allows automation to process omni_sales orders instead of ramos_orders
if ($CI->db->table_exists(db_prefix() . 'cart')) {
    if (!$CI->db->field_exists('processed_for_purchase', db_prefix() . 'cart')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "cart` ADD `processed_for_purchase` TINYINT(1) NULL DEFAULT 0 COMMENT 'Whether order was included in purchase automation';");
    }

    if (!$CI->db->field_exists('processed_at', db_prefix() . 'cart')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "cart` ADD `processed_at` DATETIME NULL COMMENT 'When order was processed by automation';");
    }

    if (!$CI->db->field_exists('automation_run_id', db_prefix() . 'cart')) {
        $CI->db->query('ALTER TABLE `' . db_prefix() . "cart` ADD `automation_run_id` INT UNSIGNED NULL COMMENT 'Link to automation run';");
    }

    // Create index for processed_for_purchase if it doesn't exist
    $indexes = $CI->db->query('SHOW INDEX FROM `' . db_prefix() . "cart` WHERE Key_name = 'idx_cart_processed'")->result_array();
    if (empty($indexes)) {
        $CI->db->query('CREATE INDEX `idx_cart_processed` ON `' . db_prefix() . 'cart` (`processed_for_purchase`);');
    }
}
