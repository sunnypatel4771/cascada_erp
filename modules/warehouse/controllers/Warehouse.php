...
class warehouse extends AdminController {
	...
	public function import_file_xlsx_opening_stock() {
					...
					if (move_uploaded_file($tmpFilePath, $newFilePath)) {
						...
						for ($row = 1; $row < count($data); $row++) {
								...
								if (($flag == 0) && ($flag2 == 0)) {
									...
									// FIX: Excel import was missing unit_price, causing "Undefined array key 'unit_price'" error
									// The add_inventory_manage() function expects unit_price to check if inventory record exists
									// Fetch purchase price from item master data (tblitems.purchase_price)
									$purchase_price = $this->warehouse_model->get_purchase_price_from_commodity_code($flag_id_commodity_code);
									$rd['unit_price'] = $purchase_price ? $purchase_price : 0.00; // Default to 0.00 if not set in item master
						...
						// OLD: $message = 'Not enought rows for importing'; // Confusing message with typo - replaced with conditional messages below

						if(($total_rows_data_error > 0) || ($total_row_false != 0)){
							// FIX: Show clear error message when import has validation errors
							$message = 'Import completed with errors. Please check the error file.';
							...
							// FIX: Show clear success message with count of imported items
							$message = 'Opening stock imported successfully! ' . $total_row_success . ' items updated.';
						}
		...
		echo json_encode([
			// OLD: 'message' =>'Not enought rows for importing', // Always showed this confusing message regardless of success/failure
			'message' => isset($message) ? $message : 'Import completed', // FIX: Use dynamic message based on import result
			'total_row_success' => $total_row_success,
			'total_row_false' => $total_rows_data_error,
			'total_rows' => $total_rows_data,
			'site_url' => site_url(),
