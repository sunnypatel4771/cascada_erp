...
class Orders_model extends App_Model
{
    ...
    /**
     * Get unprocessed orders (status = 'new' AND processed_for_purchase = 0)
     *
     * @return array
     */
    public function get_unprocessed_orders(): array
    {
        $this->db->where('status', RAMOS_ORDER_STATUS_NEW);
        $this->db->where('processed_for_purchase', 0);
     ...
     * @return bool
     */
    public function mark_orders_as_processed(array $orderIds, int $automationRunId): bool
    {
        if (empty($orderIds)) {
            return false;
        }
