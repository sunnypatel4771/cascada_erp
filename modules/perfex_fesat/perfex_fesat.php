...
/*
Module Name: FE-SAT
Description: Generate CFDI 4.0 invoices with Digibox timbrado and attach results to invoices.
Version: 1.0.0
...
const PERFEX_FESAT_MODULE_ROUTE = 'fe_sat';
...
function perfex_fesat_ensure_upload_directory(): void
{
    if (!is_dir(PERFEX_FESAT_UPLOAD_DIR)) {
        if (!mkdir(PERFEX_FESAT_UPLOAD_DIR, 0755, true) && !is_dir(PERFEX_FESAT_UPLOAD_DIR)) {
            log_activity('FE-SAT could not create uploads directory: ' . PERFEX_FESAT_UPLOAD_DIR);
        }
    }
}
...
function perfex_fesat_seed_options(): void
{
    $defaults = [
        ...
        'perfex_fesat_ssl_verify_disabled' => '0',
